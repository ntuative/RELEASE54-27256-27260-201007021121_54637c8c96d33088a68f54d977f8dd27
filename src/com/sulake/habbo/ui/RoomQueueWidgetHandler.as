package com.sulake.habbo.ui
{
   ...
   public class RoomQueueWidgetHandler implements IRoomWidgetHandler
   {
      ...
      public function processEvent(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            case RoomSessionQueueEvent.const_449:
               ...
               switch(_loc2_.queueSetTarget)
               {
                  case RoomSessionQueueEvent.const_1047:
                     _loc3_ = "null";
                     break;
                  case RoomSessionQueueEvent.const_921:
                     _loc3_ = "null";
               }
               if(_loc3_ == null)
               {
               ...
               if(_loc7_.length > 1)
               {
                  if(_loc6_ && _loc2_.queueTypes.indexOf(RoomSessionQueueEvent.const_964) != -1)
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_964) + 1;
                     _loc9_ = true;
                  }
                  else
                  {
                     _loc8_ = _loc2_.getQueueSize(RoomSessionQueueEvent.const_1239) + 1;
                  }
               }
               else
               {
      ...
      {
         var _loc1_:* = [];
         _loc1_.push(RoomWidgetRoomQueueMessage.const_734);
         _loc1_.push(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_586);
         _loc1_.push(RoomWidgetRoomQueueMessage.const_567);
         return _loc1_;
      }
      ...
      public function processWidgetMessage(param1:RoomWidgetMessage) : RoomWidgetUpdateEvent
      {
         ...
         switch(param1.type)
         {
            case RoomWidgetRoomQueueMessage.const_734:
               _container.roomSession.quit();
               break;
            case RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_921);
               break;
            case RoomWidgetRoomQueueMessage.const_586:
               _container.roomSession.changeQueue(RoomSessionQueueEvent.const_1047);
               break;
            case RoomWidgetRoomQueueMessage.const_567:
               if(false)
               {
                  _container.catalog.openCatalogPage(CatalogPageName.const_216,true);
               }
         }
         return null;
      }
      ...
      {
         return [RoomSessionQueueEvent.const_449];
      }
