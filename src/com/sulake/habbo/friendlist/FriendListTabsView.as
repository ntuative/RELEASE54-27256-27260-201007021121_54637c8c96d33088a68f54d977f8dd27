package com.sulake.habbo.friendlist
{
   ...
   public class FriendListTabsView
   {
      ...
      private function refreshHeader(param1:FriendListTab, param2:IWindowContainer) : int
      {
         ...
         var _loc4_:Boolean = param1.id == FriendListTab.const_82 && !param1.newMessageArrived;
      ...
      private function refreshTabContentDims(param1:IWindowContainer) : void
      {
         ...
         var _loc7_:int = _friendList.tabs.tabContentWidth;
         var _loc8_:int = _friendList.tabs.tabContentHeight;
      ...
      private function onTabClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         for each(_loc4_ in _friendList.tabs.getTabs())
         {
         ...
         if(_loc3_.selected)
         {
            switch(_loc3_.id)
            {
               case FriendListTab.const_82:
                  _friendList.trackFriendListEvent(HabboFriendListTrackingEvent.HABBO_FRIENDLIST_TRACKIG_EVENT_FRIENDS);
                  break;
               case FriendListTab.const_341:
                  _friendList.trackFriendListEvent(HabboFriendListTrackingEvent.HABBO_FRIENDLIST_TRACKIG_EVENT_SEARCH);
                  break;
               case FriendListTab.const_84:
                  _friendList.trackFriendListEvent(HabboFriendListTrackingEvent.HABBO_FRIENDLIST_TRACKIG_EVENT_REQUEST);
            }
         }
         else
         {
            _friendList.trackFriendListEvent(HabboFriendListTrackingEvent.const_324);
         }
      }
      ...
      public function refresh(param1:String) : void
      {
         ...
         var_22.width = _friendList.tabs.tabContentWidth;
         var_22.findChildByName("bg").width = _friendList.tabs.tabContentWidth;
         var _loc2_:int = 1;
         for each(_loc3_ in _friendList.tabs.getTabs())
         {
            _loc4_ = var_22.getChildByName("flt_" + _loc3_.id) as IWindowContainer;
            if(!isTabVisible(_loc3_.id))
            {
               _loc4_.visible = false;
            }
            else
            {
               ...
               _loc4_.width = _friendList.tabs.tabContentWidth;
               _loc4_.y = _loc2_;
               _loc5_ = refreshHeader(_loc3_,_loc4_);
               refreshTabContent(_loc3_,_loc4_);
               _loc4_.height = _loc5_ + (!!_loc3_.selected ? _friendList.tabs.tabContentHeight : 0);
               _loc2_ += _loc4_.height;
            }
         }
         var_22.height = _loc2_ + 1;
         var_22.findChildByName("bg").height = var_22.height;
      ...
      private function refreshScrollBarVisibility(param1:IWindowContainer) : void
      {
         ...
         var _loc7_:int = _friendList.tabs.tabContentWidth - 10;
         var _loc8_:int = _loc7_ - 22;
         var _loc9_:int = !!_loc5_ ? int(_loc8_) : int(_loc7_);
         _loc3_.visible = _loc5_;
         _loc4_.width = _loc9_;
         change(_loc4_,_loc9_);
      }
      
      private function isTabVisible(param1:int) : Boolean
      {
         if(param1 != FriendListTab.const_84)
         {
            return true;
         }
