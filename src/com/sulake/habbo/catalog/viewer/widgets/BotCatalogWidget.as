package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class BotCatalogWidget extends CatalogWidget implements ICatalogWidget, IItemGrid, IAvatarImageListener
   {
      ...
      private var var_1303:Map;
      
      private var var_738:Offer;
      
      public function BotCatalogWidget(param1:IWindowContainer)
      {
         super(param1);
         var_1024 = _window.findChildByName("botGrid") as IItemGridWindow;
         var_1569 = new Map();
      }
      ...
      public function avatarImageReady(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc8_:* = null;
         var _loc2_:int = var_1569.getValue(param1);
         if(true)
         {
            return;
         }
         for each(_loc3_ in page.offers)
         {
            ...
            {
               _loc4_ = var_1303.getValue(_loc3_.productContainer.firstProduct.extraParam.toLowerCase());
               ...
               if(_loc7_)
               {
                  _loc7_.setDirection(AvatarSetType.const_30,4);
                  _loc8_ = _loc7_.getImage(AvatarSetType.const_30,true);
                  _loc6_.bitmap.copyPixels(_loc8_,_loc8_.rect,new Point((_loc6_.width - _loc8_.width) / 2,_loc6_.height - _loc8_.height));
                  _loc6_.invalidate();
                  _loc7_.dispose();
                  if(var_738.offerId == _loc2_)
                  {
                     updatePreviewImage();
                  }
               }
      ...
      {
         var _loc6_:* = null;
         var _loc2_:IWindowContainer = page.viewer.catalog.windowManager.buildFromXML(var_1391) as IWindowContainer;
         var_1024.addGridItem(_loc2_);
         param1.productContainer.view = _loc2_;
         var _loc3_:String = var_1303.getValue(param1.productContainer.firstProduct.extraParam.toLowerCase());
      ...
      {
         var _loc1_:String = var_1303.getValue(var_738.productContainer.firstProduct.extraParam.toLowerCase());
         var _loc2_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createAvatarImage(_loc1_,AvatarScaleType.const_49);
         if(_loc2_)
         {
            _loc2_.setDirection(AvatarSetType.const_30,4);
            setPreviewImage(_loc2_.getImage(AvatarSetType.const_30,true));
            _loc2_.dispose();
         }
      }
      ...
      {
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_738,page);
      }
      ...
      private function setProductData() : void
      {
         ...
         if(_loc2_ != null)
         {
            _loc1_.registerParameter("catalog.purchase.price.credits+pixels","credits",String(var_738.priceInCredits));
            _loc1_.registerParameter("catalog.purchase.price.credits+pixels","pixels",String(var_738.priceInPixels));
            _loc2_.text = "${catalog.purchase.price.credits+pixels}";
         }
         var _loc3_:IProductData = page.viewer.catalog.getProductData(var_738.localizationId);
