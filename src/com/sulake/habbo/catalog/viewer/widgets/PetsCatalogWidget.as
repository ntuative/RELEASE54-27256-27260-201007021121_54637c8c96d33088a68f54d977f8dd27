package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_409 >= var_309.length)
         {
            return "";
         }
         var petColor:IPetColor = var_309[var_409] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_604[var_1513] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_409 = param1.index;
         if(var_409 < 0 || var_409 > var_309.length)
         {
            var_409 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_603.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_131,onTextWindowEvent);
         var_603 = new Map();
         ...
         var_858 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_309 = [];
         var_604 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_858,this);
               if(_loc11_)
               {
                  var_604 = _loc11_.breeds;
                  var_309 = _loc11_.colors;
                  if(var_309 == null)
                  {
                     var_309 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_604.length)
         {
            _loc12_ = var_604[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_858,_loc12_.id),getRaceLocalizationKey(var_858,_loc12_.id)));
            _loc6_++;
         }
         var_603.add(var_858,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_848,onApproveNameResult);
         _loc5_ = var_603.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_309)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_603.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_409 >= 0 && var_409 < var_309.length)
         {
            _loc7_ = var_309[var_409] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_604[var_1513] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_858,_loc3_.id,_loc2_,AvatarScaleType.const_49,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_283,AvatarAction.const_757);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_30,2);
         setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_30),true);
         _loc4_.dispose();
