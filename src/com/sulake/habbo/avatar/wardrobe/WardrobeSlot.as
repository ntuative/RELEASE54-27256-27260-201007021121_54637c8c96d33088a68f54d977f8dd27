package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_337:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         _view.visible = false;
         var_337 = _view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(var_469 && var_1454)
         {
            _loc5_ = _controller.avatarRenderManager.createAvatarImage(figure,AvatarScaleType.SMALL,var_677,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_30,parseInt(FigureData.const_609));
         ...
         if(var_337)
         {
            if(false)
            {
               var_337.bitmap.dispose();
            }
            var_337.bitmap = new BitmapData(var_337.width,var_337.height,true,0);
            _loc7_ = (0 - _loc1_.width) / 2;
            _loc8_ = (0 - _loc1_.height) / 2;
            var_337.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               var_469 = _controller.figureData.getFigureString();
               var_677 = _controller.gender;
               _controller.handler.saveWardrobeOutfit(var_1695,this);
               updateView();
               break;
            case "get_button":
               if(var_469)
               {
                  _controller.loadAvatarInEditor(var_469,var_677,_controller.clubMemberLevel);
               }
         }
      }
