package com.sulake.habbo.avatar.pets
{
   public class PetData implements IPetData
   {
      ...
      public function PetData(param1:XML)
      {
         ...
         super();
         _name = param1.@name;
         var_434 = parseInt(param1.@species);
         var_1923 = Boolean(parseInt(param1.@disableheadturn));
         var_154 = param1.@scale;
         _colors = new Array();
         var_1288 = new Array();
         Logger.log("Found Pet Pack: " + _name);
         for each(_loc2_ in param1.breeds.breed)
         {
            var_1288.push(new Breed(_loc2_));
         }
      ...
      {
         var _loc2_:* = null;
         for each(var _loc5_ in var_1288)
         {
            _loc2_ = _loc5_;
            _loc5_;
            if(_loc2_.id == param1)
