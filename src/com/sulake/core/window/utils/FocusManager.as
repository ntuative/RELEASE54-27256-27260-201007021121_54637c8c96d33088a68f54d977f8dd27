package com.sulake.core.window.utils
{
   ...
   public class FocusManager extends ChildEntityArrayReader implements IWindowFocusManagerService
   {
      ...
      public function FocusManager(param1:DisplayObject)
      {
         ...
         var_205.addEventListener(Event.ACTIVATE,onActivateEvent);
         var_205.addEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
         var_205.addEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
         var_205.addEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
         super();
      }
      ...
      public function dispose() : void
      {
         if(!_disposed)
         {
            var_205.removeEventListener(Event.ACTIVATE,onActivateEvent);
            var_205.removeEventListener(FocusEvent.FOCUS_OUT,onFocusEvent);
            var_205.removeEventListener(FocusEvent.KEY_FOCUS_CHANGE,onFocusEvent);
            var_205.removeEventListener(FocusEvent.MOUSE_FOCUS_CHANGE,onFocusEvent);
            var_205 = null;
            _disposed = true;
            var_34 = null;
         }
