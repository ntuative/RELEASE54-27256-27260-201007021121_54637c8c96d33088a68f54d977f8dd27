package
{
   ...
   public class HabboMain extends Sprite
   {
      ...
      private static const const_770:Number = 0.71;
      ...
      private function updateLoadingBar(param1:DisplayObjectContainer, param2:Number) : void
      {
         ...
         var _loc12_:Number = 1 - const_770 + var_30.getNumberOfFilesLoaded() / _loc11_ * const_770;
         _loc8_.x = 1 + 1;
         _loc8_.y = 1 + 1;
      ...
      private function prepareCore() : void
      {
         ...
         else
         {
            ...
            _loc1_.addEventListener(AssetLoaderEvent.const_39,configLoadedHandler);
         }
      }
      ...
      private function configLoadedHandler(param1:AssetLoaderEvent) : void
      {
         ...
         _loc2_.removeEventListener(AssetLoaderEvent.const_39,configLoadedHandler);
         ...
         {
            Habbo.reportCrash("Failed to download external configuration document " + _loc2_.assetLoader.url + "!",Core.const_159,null);
         }
      }
      ...
      private function initializeCore() : void
      {
         ...
         catch(error:Error)
         {
            ...
            Core.crash("Failed to initialize the core: " + error.message,Core.const_882,error);
         }
      }
      ...
      protected function onAddedToStage(param1:Event = null) : void
      {
         ...
         {
            Habbo.trackLoginStep(Habbo.const_22);
            Habbo.reportCrash("Failed to prepare the core: " + error.message,Core.const_882,error);
            Core.dispose();
         }
      }
      ...
      private function setupCoreConfigFromLoader(param1:AssetLoaderStruct) : void
      {
         ...
         {
            Habbo.reportCrash("Download external configuration document is null!",Core.const_159,null);
         }
         if(var_30)
         {
         ...
         {
            Habbo.reportCrash("Core vanished while downloading config document!",Core.const_159,null);
         }
      }
